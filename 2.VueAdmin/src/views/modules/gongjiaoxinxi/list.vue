			...
			<el-form class="center-form-pv" :style='{"border":"0px solid #ceddee","minHeight":"100px","padding":"10px 0px","boxShadow":"0 0px 0px 0px rgba(100,100,100,.05)","margin":"0px","overflow":"hidden","borderRadius":"20px 20px","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"center"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","margin":"0 0px 0px 0","borderRadius":"0px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-end","order":"2"}' >
					<div :style='{"margin":"0 5px 10px 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 5px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">公交编号</label>
						<el-input v-model="searchForm.gongjiaobianhao" placeholder="公交编号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 5px 10px 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 5px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">公交班次</label>
						<el-input v-model="searchForm.gongjiaobanci" placeholder="公交班次" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 5px 10px 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 5px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">起点站</label>
						<el-input v-model="searchForm.qidianzhan" placeholder="起点站" @keydown.enter.native="search()" clearable></el-input>
					</div>
					...
					<div :style='{"margin":"0 5px 10px 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 5px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">终点站</label>
						<el-input v-model="searchForm.zhongdianzhan" placeholder="终点站" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","padding":"0 20px 0 0","margin":"0px 0 0px","color":"#333","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"auto","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('gongjiaoxinxi','新增')" type="success" @click="addOrUpdateHandler()">
					...
					<el-button class="del" v-if="isAuth('gongjiaoxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"0px","boxShadow":"inset 0px 0px 0px 0px #E8EFF6","borderColor":"#d2d0d0","margin":"0 0 0px","borderRadius":"0px","background":"rgba(255,255,255,.8)","borderWidth":"1px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="gongjiaobianhao"
						label="公交编号">
						<template slot-scope="scope">
							{{scope.row.gongjiaobianhao}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="gongjiaobanci"
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="途经站点">
						<template slot-scope="scope">
							{{scope.row.tujingzhandian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="shoubancheshijian"
						label="首班车时间">
						<template slot-scope="scope">
							{{scope.row.shoubancheshijian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="mobancheshijian"
						label="末班车时间">
						<template slot-scope="scope">
							{{scope.row.mobancheshijian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="fachejiange"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('gongjiaoxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="edit" v-if=" isAuth('gongjiaoxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('gongjiaoxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.gongjiaobianhao!='' && this.searchForm.gongjiaobianhao!=undefined){
            params['gongjiaobianhao'] = '%' + this.searchForm.gongjiaobianhao + '%'
          }
           if(this.searchForm.gongjiaobanci!='' && this.searchForm.gongjiaobanci!=undefined){
            params['gongjiaobanci'] = '%' + this.searchForm.gongjiaobanci + '%'
          }
           if(this.searchForm.qidianzhan!='' && this.searchForm.qidianzhan!=undefined){
            params['qidianzhan'] = '%' + this.searchForm.qidianzhan + '%'
          }
           ...
           if(this.searchForm.zhongdianzhan!='' && this.searchForm.zhongdianzhan!=undefined){
            params['zhongdianzhan'] = '%' + this.searchForm.zhongdianzhan + '%'
          }
	...
	gongjiaoxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'gongjiaoxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
