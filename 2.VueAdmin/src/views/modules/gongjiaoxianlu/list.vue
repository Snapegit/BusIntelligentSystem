<template>
	<div class="main-content" :style='{"minHeight":"calc(100vh - 200px)","padding":"20px 0 30px","margin":"0 auto","color":"#666","background":"none","width":"calc(100% - 60px)","fontSize":"16px","height":"100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #ceddee","minHeight":"100px","padding":"10px 0px","boxShadow":"0 0px 0px 0px rgba(100,100,100,.05)","margin":"0px","overflow":"hidden","borderRadius":"20px 20px","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"center"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","margin":"0 0px 0px 0","borderRadius":"0px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-end","order":"2"}' >
					<div :style='{"margin":"0 5px 10px 0","fontSize":"inherit","display":"inline-block"}' class="select" label="线路" prop="xianlu">
						<label :style='{"margin":"0 5px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">线路</label>
						<el-select clearable v-model="searchForm.xianlu" placeholder="请选择线路" >
							<el-option v-for="(item,index) in xianluOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 5px 10px 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 5px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">始发站点</label>
						<el-input v-model="searchForm.shifazhandian" placeholder="始发站点" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 5px 10px 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 5px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">终点站点</label>
						<el-input v-model="searchForm.zhongdianzhandian" placeholder="终点站点" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 5px 10px 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 5px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">换乘信息</label>
						<el-input v-model="searchForm.huanchengxinxi" placeholder="换乘信息" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","padding":"0 20px 0 0","margin":"0px 0 0px","color":"#333","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"auto","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('gongjiaoxianlu','新增')" type="success" @click="addOrUpdateHandler()">
					...
					<el-button class="del" v-if="isAuth('gongjiaoxianlu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('gongjiaoxianlu','公交车次')" type="success" @click="chartDialog1()">
					...
					<el-button class="btn18" v-if="isAuth('gongjiaoxianlu','公交线路')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","color":"#327e33","left":"10px","display":"inline-block","fontSize":"24px","position":"absolute","fontWeight":"500"}'></span>
						公交线路
					</el-button>
				</el-row>
			...
			<div :style='{"padding":"0px","boxShadow":"inset 0px 0px 0px 0px #E8EFF6","borderColor":"#d2d0d0","margin":"0 0 0px","borderRadius":"0px","background":"rgba(255,255,255,.8)","borderWidth":"1px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="luxiantupian" width="200" label="路线图片">
						<template slot-scope="scope">
							<div v-if="scope.row.luxiantupian">
								<img v-if="scope.row.luxiantupian.substring(0,4)=='http'" :src="scope.row.luxiantupian.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.luxiantupian.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('gongjiaoxianlu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="edit" v-if=" isAuth('gongjiaoxianlu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="view" v-if="isAuth('gongjiaoxianlu','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan5" :style='{"margin":"0 0 0 4px","fontSize":"24px","position":"absolute","color":"#327e33","left":"0","height":"auto"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('gongjiaoxianlu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="checiChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="xianluChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var checiChart1 = echarts.init(document.getElementById("checiChart1"),'macarons');
        this.$http({
            url: "gongjiaoxianlu/group/checi",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var xianluChart2 = echarts.init(document.getElementById("xianluChart2"),'macarons');
        this.$http({
            url: "gongjiaoxianlu/group/xianlu",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.xianlu!='' && this.searchForm.xianlu!=undefined){
            params['xianlu'] = this.searchForm.xianlu
          }
           ...
           if(this.searchForm.huanchengxinxi!='' && this.searchForm.huanchengxinxi!=undefined){
            params['huanchengxinxi'] = '%' + this.searchForm.huanchengxinxi + '%'
          }
	...
	this.$router.push({path:'/discussgongjiaoxianlu',query:{refid:id}});
    },
    // 下载
    download(file){
	...
	gongjiaoxianlustatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'gongjiaoxianlu/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
