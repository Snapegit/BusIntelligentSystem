...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 2%","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 20px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>始发站点：</div>
        <el-input v-model="formSearch.shifazhandian" placeholder="始发站点" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 20px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>终点站点：</div>
      ...
      <el-form-item :style='{"margin":"0 10px 20px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>换乘信息：</div>
        <el-input v-model="formSearch.huanchengxinxi" placeholder="换乘信息" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('gongjiaoxianlu','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 25px","margin":"0 10px 20px 0","outline":"none","color":"#fff","borderRadius":"0","background":"#57A7A5","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/gongjiaoxianluAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"100%","margin":"0 0 10px","background":"none","order":"4"}'>
		...
		<div :style='{"width":"100%","padding":"0 0 0 2%","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}' class="list list4 index-pv1">
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"margin":"20px 2% 10px 0","overflow":"hidden","background":"#fff","width":"31.3%","minWidth":"320px","position":"relative","height":"280px"}' class="item">
			<img @click.stop="imgPreView(item.luxiantupian)" v-if="item.luxiantupian && item.luxiantupian.substr(0,4)=='http'" :src="item.luxiantupian.split(',')[0]" class="image" />
			<img @click.stop="imgPreView(baseUrl + (item.luxiantupian?item.luxiantupian.split(',')[0]:''))" v-else :src="baseUrl + (item.luxiantupian?item.luxiantupian.split(',')[0]:'')" class="image" />
			<div class="content">
		      <div :style='{"padding":"0","overflow":"hidden","color":"#fff","flex":"1","display":"-webkit-box","width":"50%","lineHeight":"24px","fontSize":"14px","-webkit-box-orient":"vertical","-webkit-line-clamp":"5","height":"120px"}'>{{item.tujingzhandian}}</div>
		      <div class="info" :style='{"width":"50%","padding":"0 0 0 10px","flexWrap":"wrap","flex":"1","display":"flex","height":"100%"}'>
				<div :style='{"overflow":"hidden","color":"#fff","display":"-webkit-box","width":"100%","lineHeight":"1.5","fontSize":"14px","-webkit-box-orient":"vertical","-webkit-line-clamp":"3","order":"1"}' class="name ">{{item.gongjiaochehao}}</div>
    ...
    created() {
		...
		this.$http.get('option/xianlu/xianlu').then(res => {
			if (res.data.code == 0) {
				...
				this.selectOptionsList.push({name:'线路',list:this.xianluOptions,tableName: 'xianlu',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.indexQueryCondition) {
          searchWhere.xianlu = '%' + this.indexQueryCondition + '%';
          this.indexQueryCondition = '';
        }
        if (this.formSearch.xianlu != '') searchWhere.xianlu = this.formSearch.xianlu;
        ...
        if (this.formSearch.huanchengxinxi != '') searchWhere.huanchengxinxi = '%' + this.formSearch.huanchengxinxi + '%';
        if (this.curFenlei != '全部') searchWhere.xianlu = this.curFenlei;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`gongjiaoxianlu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/gongjiaoxianluDetail', query: params});
      },
